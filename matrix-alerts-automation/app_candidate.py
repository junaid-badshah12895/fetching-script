...
from src.config_candidate import (
    candidate_matrix_channel_id,
    channel_mapping
)
...
async def main():
    try:
        ...
        # Process candidate_matrix channel with configured date/time
        process_candidate_matrix_alerts = await process_matrix_alerts(
            channel_id=candidate_matrix_channel_id,
            start_datetime=start_tuple,
            end_datetime=end_tuple
        )

        # Separate customer and internal alerts
        separate_candidate_matrix = await seprate_customers_and_internal_alerts(process_candidate_matrix_alerts)

        customer_candidate_matrix_alerts = separate_candidate_matrix.get("customer_alerts", [])
        notell_candidate_matrix_alerts = separate_candidate_matrix.get("internal_alerts", [])

        customer_alerts_count = {
            "Overall Customer candidate_matrix Alerts": len(customer_candidate_matrix_alerts),
        }
        notell_alerts_count = {
            "Overall Notell candidate_matrix Alerts": len(notell_candidate_matrix_alerts),
        }

        print("Customer Alerts Count:", customer_alerts_count)
        print("Notell Alerts Count:", notell_alerts_count)

        with open("customer_candidate_manual_alerts.json", "w") as f:
            json.dump(customer_candidate_matrix_alerts, f, indent=4)
        with open("notell_candidate_manual_alerts.json", "w") as f:
            json.dump(notell_candidate_matrix_alerts, f, indent=4)
